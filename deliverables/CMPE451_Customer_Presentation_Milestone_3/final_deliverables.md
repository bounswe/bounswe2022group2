...
The app has text and image annotations for both the web and mobile versions. The text annotations allow users to add notes and comments to the posts of the learning spaces in the app, while the image annotations allow users to highlight and annotate specific areas of images within the posts. These annotations can be used to provide additional context, clarification, or feedback on the content. Users can see their own and other users' annotaions. On mobile version, annotations of the current user are shown on top of the screen, when "see annotations" section is activated. Text annotations are shown by highlights on related text, while image annptations have square shape identifiers on the image. Both the text and image annotations are available on both the web and mobile versions of the app, allowing users to access and utilize these features regardless of the platform they are using. Since there is full backend connection, annotations made by each platform can be accessed by each platform.
